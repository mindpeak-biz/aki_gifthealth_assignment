...
class SortBy(Enum):
    NONE = "keep order as ingested"
    ALPHABETICAL = "alphabetical"
    DECREASING_INCOME = "decreasing income" 
    DECREASING_FILLS = "decreasing fills"
...
class PatientUtility:
    ...
    @classmethod
    def sort_aggregate_patient_by_decreasing_income(cls, patient_dict):
        return dict(sorted(patient_dict.items(), key=lambda item: item[1].total_income_to_pharmacy, reverse=True))
    
    @classmethod
    def sort_aggregate_patient_by_decreasing_fills(cls, patient_dict):
        return dict(sorted(patient_dict.items(), key=lambda item: item[1].total_fills, reverse=True))